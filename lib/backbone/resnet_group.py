...
class ResNet_Group(nn.Module):
    def __init__(
        ...
        self.layermany = self._make_layer(num_blocks[3],
            512,
            stride=last_layer_stride,)

        self.inplanes = inplane
        self.layermedium = self._make_layer(num_blocks[3],
            512,
            stride=last_layer_stride,)

        self.inplanes = inplane
        self.layerfew = self._make_layer(num_blocks[3],
            512,
            stride=last_layer_stride,)
    ...
    def load_model(self, pretrain):
        ...
        for k, v in pretrain_dict.items():
            ...
            if "last_linear" not in k and "classifier" not in k and "linear" not in k and "fd" not in k:
                ...
                new_dict[k] = v
            """
            if "fc" not in k and "classifier" not in k and 'layer4' not in k:
                k = k.replace("backbone.", "")
                new_dict[k] = v
            if 'layer4' in k:
                k = k.replace("layer4", "layermany")
                new_dict[k] = v
                k = k.replace("layer4", "layermedium")
                new_dict[k] = v
                k = k.replace("layer4", "layerfew")
                new_dict[k] = v
            """
