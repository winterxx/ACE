...
class ResNet_Group(nn.Module):
    ...
    def __init__(self, block, layers, use_modulatedatt=False, use_fc=False, dropout=None):
        ...
        self.layermany = self._make_layer(block, 512, layers[3], stride=2)

        self.inplanes = inplane
        self.layermedium = self._make_layer(block, 512, layers[3], stride=2)

        self.inplanes = inplane
        self.layerfew= self._make_layer(block, 512, layers[3], stride=2)
    ...
    def forward(self, x, **kwargs):
        ...
        if 'layer' in kwargs and kwargs['layer'] == 'layer4':
            out = kwargs['coef']*out+(1-kwargs['coef'])*out[kwargs['index']]
        many_out = self.layermany(out)
        medium_out = self.layermedium(out)
        few_out = self.layerfew(out)

        return [few_out, medium_out, many_out, out]
...
def res10_group(cfg, use_selfatt=False, use_fc=False, dropout=None, stage1_weights=False, dataset=None, test=False,
          pretrain=False,
          pretrained_model =  None,
          last_layer_stride = None,
          flag=None):
    print('Loading Scratch ResNet 10 Feature Model.')
    resnet10 = ResNet_Group(BasicBlock, [1, 1, 1, 1], use_modulatedatt=use_selfatt, use_fc=use_fc, dropout=None)
    if not test:
        if stage1_weights:
            assert(dataset)
